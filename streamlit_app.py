...
# Load December data
dec_file_path = r"Roundtable Data/past_bookings_Dec23 (1).csv"
...
# Load June vehicles data
june_subs_file_path = r"Subscription Data/possible_subscribers_Jun23.csv"
df_subs_june = pd.DataFrame(load_monthly_data(june_subs_file_path))

# Load July vehicles data
july_subs_file_path = r"Subscription Data/possible_subscribers_Jul23.csv"
...
#df_month.to_csv(r"C:\Users\DELL\Downloads\finalstream\finalstream\scamui.csv")
...
df_month['Actual Date'] = df_month['Reach Date'].fillna('') + df_month['Reach date'].fillna('') + df_month['Aug Reach Date'].fillna('') + df_month['Nov Reach Date'].fillna('') + df_month['Dec Reach Date'].fillna('')
...
df_month['E-pod Arrival Time @ Session location'] = df_month['Reach time'].fillna('').astype(str) + df_month['Arrival Time'].fillna('').astype(str) + df_month['Aug Reach Time'].fillna('').astype(str) + df_month['Nov Reach Time'].fillna('').astype(str) + df_month['Dec Reach Time'].fillna('').astype(str)
...
#vehicle_df.to_csv(r"C:\Users\DELL\Downloads\finalstream\finalstream\sk.csv")
...
tab1, tab2, tab3, tab5, tab6 = st.tabs(
    ["Executive Dashboard", "Charge Pattern Insights", "EPod Stats", "Subscription Insights", "Geographical Insights"])
